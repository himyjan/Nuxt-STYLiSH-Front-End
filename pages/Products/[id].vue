...
const product = ref<ProductDetailsData>();

watchEffect(() => {
  async function fetchtDetails() {
    const { data } = await api.getProduct(id.value as string);
    product.value = data;
    console.log(data);
  }
  fetchtDetails();
...
<template>
  <Wrapper v-if="product">
    ...
    <Images>
      <Image :src="image" :key="index" v-for="(image, index) in product!.images" />
    </Images>
  </Wrapper>
</template>
